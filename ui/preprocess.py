...
# Словарь сокращений и их полных форм для расширения
...
# Расширяем сокращения при помощи CONTRACTIONS
...
# Удаляем все числовые значения и лишние пробелы
def remove_numbers(text: str) -> str:
    text = re.sub(r"\d+", " ", text)
    return re.sub(r"\s+", " ", text).strip()

# Удаляем все символы, кроме букв и пробелов, чтобы оставить только слова
...
# Удаляем последовательные повторяющиеся слова
...
# Объединяем все в общую функцию для очистки текста, получаем токены для модели DistilBERT
